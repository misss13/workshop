...
W ramach monitorowania działań na systemie napiszmy więc własną regułę do monitorowania pliku authorized_keys. Plik ten znajduje się w katalogu domowym użytkownika w ścieżce `~/.ssh/authorized_keys` i zawiera klucze publiczne użytkowników, którzy mogą zalogować się na tego użytkownika. Dodając do niego wpis tworzymy tym samym dostęp do tego środowiska. 

Zatem zmiany tego pliku mogą powodować dodanie komuś uprawnień do logowania na danego użytkownika. Mogą być one spowodowane dostaniem się do systemu osoby niepowołanej a ta chcąc sobie wyrobić stały dostęp do serwera doda swój klucz.

## Budowa reguły

Nasza reguła będzie się nazywać `Ssh keys added to authorized_keys`. Natomiast opis naszej reguły jak wyżej:
...
- oraz ((plik musi być w ścieżce która zawiera `/.ssh/` i musi pasować do regexu `'/home/*/.ssh/*'`) lub nazwa ścieżki zaczyna się od `/root/.ssh`) - `and (user_ssh_directory or fd.name startswith /root/.ssh)`
- oraz (nazwa pliku kończy się na `authorized_keys` lub `authorized_keys2`) - `and (fd.name endswith authorized_keys or fd.name endswith authorized_keys2)`
- oraz nazwa procesu, który zrobił modyfikację pliku musi istnieć
...
## Uzupełnienie pliku values.yaml

Proszę otworzyć zakładkę Editor i wprowadzić powyższe wymagania dla reguły, finalnie plik powinien mieć budowę (zmieniane i modyfikowane powinno być wszystko co jest poniżej linijki `  custom-rules.yaml: |-` aż do `falcosidekick:`)
...
Następnie należy przeładować nasze pody żeby, usunęły się stare i utworzyły nowe z nową konfiguracją, warto zauważyć że zmieniliśmy tylko wymagania dla daemonsetu falco (daemonset zarządza podami w tym wypadku 2ma podami falco), nie musimy restartować 2 podów od falcosidekick, ponieważ ich konfiguracja nie uległa zmianie.
...
Na discordzie kanał `soc-project-team` powinien przyjść alert.
