...
Przy uruchomieniu falco bez wskazania mu pliku z regułami zostanie użyty [domyślny zestaw reguł](https://github.com/falcosecurity/rules/blob/main/rules/falco_rules.yaml).
...
Taki system call jest wykorzystywany do odczytu pliku.
```
- macro: open_read
  condition: (evt.type in (open,openat,openat2) and evt.is_open_read=true and fd.typechar='f' and fd.num>=0)
```

Spis plików które zostały oznaczone jako wrażliwe pliki
```
...
W powyższym makro zostało zrobione odwołanie do listy zawierającej nasz plik `/etc/shadow`
```
- list: sensitive_file_names
  items: [/etc/shadow, /etc/sudoers, /etc/pam.conf, /etc/security/pwquality.conf]
```
Makro wymagające żeby proces posiadał nazwę.
```
...
- rule: Read sensitive file untrusted
  ...
  condition: >
    open_read 
    and (fd.name in [/etc/shadow, ...])
    and not proc.name in ("<NA>","N/A")
